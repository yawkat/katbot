...
class FunctionsTest {
    ...
    fun `lt match`() {
        assertEquals(
                Functions.NumberCompare.evaluate(LazyExpressionList(vm, Parser.parse("lt -1 0.5 1")), Function.EvaluationMode.NORMAL),
                listOf("true")
        )
    }
    ...
    fun `lt mismatch`() {
        assertEquals(
                Functions.NumberCompare.evaluate(LazyExpressionList(vm, Parser.parse("lt -1 0.5 0.5 1")), Function.EvaluationMode.NORMAL),
                listOf("false")
        )
    }
}